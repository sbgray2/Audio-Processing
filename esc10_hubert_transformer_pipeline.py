...
# Training
def train_transformer_classifier(model, train_loader, val_loader, device='cpu', epochs=10):
    model.to(device)
...
if __name__ == "__main__":
    ...
    model = TransformerClassifier(input_dim=input_dim, num_classes=len(label_map))
    train_transformer_classifier(model, train_loader, test_loader, device=device, epochs=200)

    evaluate_model(model, test_loader, label_map, device=device, log_file="hubert_transformer_eval_log.txt")
